...
def ImageGetter(suf='', recurse=True, include=None):
    ...
    purpose:
    - just like FileGetter, ImageGetter does exactly the same for image files. 
    """
...
test_eq(len(get_files(path/'train', extensions='.png', recurse=True, include='3')), len(ImageGetter(  'train', recurse=True, include='3')(path)))

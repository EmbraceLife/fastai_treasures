...
# add args into decorators
def repeat(num_times): # to introduce args
    def decorator_repeat(func): # the proper decorator func
        @functools.wraps(func)
        def wrapper_repeat(*args, **kwargs): # the wrapper func
            for _ in range(num_times):
                value = func(*args, **kwargs)
            return value
...
# more freedom with or without args 
...
# classes as decorators to maintain states
## a normal class to maintain state
class Counter:
...
import functools
# The .__init__() method must store a reference to the function 
# and can do any other necessary initialization. The .__call__() method will be 
# called instead of the decorated function. It does essentially the same thing
# as the wrapper() function in our earlier examples. 
class CountCalls:
    
    @functools.wraps(func)
    def __init__(self, func):
        # you need to use the functools.update_wrapper() function 
